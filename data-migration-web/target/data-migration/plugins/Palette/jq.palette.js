...
function paletteTools(eid/*需要触发调色板元素ID*/){
    ...
    /*异步获取颜色值*/
    $.ajax({url:'../plugins/Palette/color.htm',type:'get',cache:false,async:false,
            error:function(){ alert('Error Message:File not found color.htm...(jq.palette.js)'); },
            success:function(data){                        
                var colors = data.split(' ');                
                colorBox.push('<div id="'+ paletteID +'" style="position:absolute;display:none;background:#fff; width:240px; _width:242px;  height:180px; border:1px solid #ccc; z-index:3;"><ul style=" padding:0px; margin:0px; float:left; list-style: none; ">');
...
paletteTools.prototype.show = function(args/*可设置调色板上左位置传值方式为{top:'1px',left:'1px'}*/){
    ...
    $('#'+ pid +' li').mouseover(function(){    
        _this.setObjColor($(this).attr('colorVal'));
    })
    /*最终选取颜色点击*/
    .click(function(){
        _this.setObjColor($(this).attr('colorVal'));
...
paletteTools.prototype.setObjColor = function(selectedColor) {
	var obj = this.Eobj;
